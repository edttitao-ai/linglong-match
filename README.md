...
| 技能 | 灵力 | 效果 |
|---|---|---|
| **如意锤** | 100 | 敲掉任意一格，对障碍造成 **2 点**伤害（一锤破开石锁）；若该格是特殊块则引爆 |
| **换天** | 70 | 全盘重排并保证有解；**盘面本来已无解时免费** |
| **灵犀一点** | 130 | 把任意一格变成你指定的颜色，随即按正常规则结算由此产生的三连（所以会自然连锁、自然造特殊块） |
| **移山** | 220 | 以目标格为中心的**整行 + 整列十字爆破**，并链式引爆波及到的特殊块 |

- **四个技能都不消耗步数**——代价只有灵力。这是「攒 → 爆」的情绪闭环。
- **灵力只能玩出来，不能买**（守住下面那条铁律）：每关开局送 90 点，其余靠消除（每块 +1）、
  连锁（每层 +6）、破障（+6）、引爆（+5）、组合（+15）积攒，上限 240。满槽后溢出按 1:2 折算成分数，
  不让你有「不敢花就浪费」的负罪感。
- **价格是量出来的，不是拍的**：一关的灵力总收入约 230~350，所以技能定价在 70~220 这一档，
  一关大约**放 2~3 次**。早先价格是 25~60，等于一关能放 4~5 次大招——实测「无脑砸移山」把
  70 关的平均胜率从 70% 推到 **99.8%**，难度直接消失；重新定价后同一个「无脑」打法只到 **90.5%**，
  而且剩下的少数几关本来就是教学关。
  移山单独定到 220 是因为它在清障关里一次能打掉整条路径上的障碍，按 160 定价时仍有 13 个清障关被推过
  「太简单」红线。这些都用 `node tools/balance.cjs --skills=max` 反复量过（它就是「无脑砸最贵技能」的模型）。
...
- **强度闸门**：`node tools/balance.cjs --skills=max` 模拟「无脑砸最贵的技能」。
  它是这批定价的主要把关口径——**技能不能强到无脑就能过**。
  重新定价后的实测（同一批种子，70 关）：

  | 打法 | 无技能 | 用技能 | 说明 |
  |---|---|---|---|
  | 无脑砸移山（上界） | 70.4% | **90.5%** | 定价 25~60 时是 **99.8%**，等于作弊 |
  | 卡住才用（策略） | 70.4% | 73.9% | 够用，但不是万能 |
  | 会乱下的弱玩家 | 58.1% | 63.2% | 救回 9 关，仍有一成的关卡过不去 |

  要再调强度就改 `CFG.QI` 与 `CFG.SKILLS` 的价格，然后跑这三条命令复验；
  关卡级还可以用 `skills: { qiStart, qiScale }` 覆写。
...
```bash
node tools/selftest.cjs --turns=20000   # 逻辑自检：205 项断言 + 两万回合随机模拟
