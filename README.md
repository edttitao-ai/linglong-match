...
- **看得懂才用得上**（技能最容易犯的错不是做得弱，是玩家不知道它是干什么的）：
  - **首次进关自动讲一次**：第一次拿到技能栏时会弹出「灵力 · 局内技能」，四个技能各一行
    ——图标、名字、消耗、一句话效果，外加一段总说明（灵力怎么来、**不能买**、技能不吃步数）。
    关掉才算「看过」，中途刷新还能再看到；之后不再打扰（存档里的 `seen` 标记）。
...
│   ├── board.js special.js resolver.js skills.js
...
- **浏览器冒烟**：内置 http 服务 + Chrome DevTools Protocol（Node 22+ 自带 WebSocket，
  ...
  **按真实节奏**分步演练四个技能（每步之间等动画收束，因为 `canInput()` 在结算期间会挡住第二次技能），
  ...
  最后检查技能栏没有压住棋盘最后一行——宽屏 / 窄屏 / 矮屏三种尺寸。
