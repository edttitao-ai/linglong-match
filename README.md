...
[![levels](https://img.shields.io/badge/levels-50-orange)](#-关卡设计)
...
- **50 关六章递进**：得分 → 收集 → 清障 → 混合 → 双目标 → 收官；步数与目标由模拟器反复验证，
  前四关几乎不会失败，最后两章需要认真规划特殊块与技能时机。
  第五章起每关两个目标，逼你在「清障」和「收集」之间分配步数；第六章压到三目标并换上新盘面
  （藤格 / 石带 / 藤墙 / 霜藤交错 / 银钩 / 铁壁）。
...
- **上界**：`node tools/balance.cjs --skills=max` 假设玩家把灵力全砸在移山上，此时全 50 关胜率会被推到
...
- **成就与画卷**：13 个成就（星级 / 连锁 / 引爆 / 破障 / 每日 / 连签 / 两种模式纪录）达成即自动发金币；
  累计星数还会逐层点亮一幅五层水墨画卷（15 / 40 / 75 / 115 / 150 星）。
...
```bash
node tools/selftest.cjs --turns=20000   # 逻辑自检：185 项断言 + 两万回合随机模拟
node tools/balance.cjs --runs=200       # 关卡平衡：贪心玩家模拟，输出胜率与星级建议
node tools/balance.cjs --level=20       # 只跑第 20 关
node tools/balance.cjs --from=31 --to=50 # 只跑一批（标定新关卡时用，不必每次跑完 50 关）
...
- **自检覆盖**：棋盘生成不变量（无初始三连、必有可行走法）· 随机回合模拟（棋盘永远填满、
  ...
  死局洗牌必定产出可玩棋盘 · 50 关数据合法性（含「收集目标颜色必须在该关色池内」这类容易写错的检查）·
...
- **平衡验证**：用一个会优先照顾目标、破障与特殊块的贪心玩家跑满每关若干局，
  输出胜率、平均剩余步数与得分分位数，并给出二星 / 三星阈值建议。当前 50 关的胜率曲线为：
